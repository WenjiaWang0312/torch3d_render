...
from t3drender.render.shaders import NormalShader, DepthShader, OpticalFlowShader
...
def render_rgb(meshes, device, resolution=(512, 512), hfov=90, cameras=None, batch_size=30, verbose=False):
    K = torch.eye(3, 3)[None]
    h, w = resolution
    mesh_renderer = MeshRenderer(resolution=resolution, shader=SoftPhongShader())

    if cameras is None:
        hfov = torch.deg2rad(torch.tensor(hfov,))
        focal_length = w / (2 * torch.tan(hfov / 2))
        K[:, 0, 0] = focal_length
        K[:, 1, 1] = focal_length
        K[:, 0, 2] = w / 2
        K[:, 1, 2] = h / 2
...
def render_depth_perspective(meshes, device, resolution=(512, 512), hfov=90, cameras=None, batch_size=30, no_grad=True, verbose=False):
    K = torch.eye(3, 3)[None]
    h, w = resolution
...
def render_flow_perspective(meshes_source, meshes_target, device, resolution=(512, 512), hfov=90, cameras=None, no_grad=True, batch_size=30, verbose=False):
    K = torch.eye(3, 3)[None]
    h, w = resolution
...
def render_normal(meshes, device, resolution=(512, 512), fov=90, cameras=None, batch_size=30, verbose=False):
    K = torch.eye(3, 3)[None]
    h, w = resolution
