...
class BaseLights(TensorProperties):
...
class AmbientLights(_AmbientLights, BaseLights):
...
class PointLights(_PointLights, BaseLights):

    def __getitem__(self, index: Union[int, slice]):
        return super(_PointLights, self).__getitem__(index)


class DirectionalLights(_DirectionalLights, BaseLights):
